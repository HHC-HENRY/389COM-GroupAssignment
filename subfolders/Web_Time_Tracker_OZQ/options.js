      ...
      Object.keys(gd.hostdata).forEach(function(dtstr) {
        ...
        if (ddate <= to && ddate >= from) {
          ...
          Object.keys(hostcounts).forEach(function(hostname) {
            var shostname =
              cfg.get('toponly') == 'true' ? host2(hostname) : hostname;
...
var drawChartWrapper = function(cb = null) {
  ...
  drawChart(gebi('timechart'), fr_d, to_d, cfg.get('type'), function() {
    if (cb) cb();
  });
};
...
window.onload = () => {
  if (localStorage.getItem('time') !== null) {
    ...
    if (time == null) {
      ...
      document.getElementById('time-1').checked = true;
    } else {
