...
function checkConfigVer(prefs) {
  ...
  if (window.downloadNotify.addonConfig === undefined) {
    ...
    var defaultVer =
      window.downloadNotify.addonConfig.config_version.defaultValue;
...
function playSoundAsset(target) {
  ...
  audioElem.src = browser.extension.getURL('assets/' + rings);
  audioElem.play();
}
...
function notify(summary, body, timeMs, filepath, downloadId) {
  ...
  function doNotifyWithPrefs(prefs) {
    if (prefs.ignore_tmp) {
      if (
        filepath.startsWith('/tmp') || // Linux / (probably) OSX
        filepath.indexOf('Local\\Temp') != -1
      ) {
        // Windoze
        return;
      }
    ...
    browser.notifications.onClicked.addListener(function(pid) {
      BoxClicked(pid, downloadId);
    });
...
function BoxClicked(pid, dw_id) {
  browser.downloads.show(dw_id);
}
...
function dlComplete(download_list) {
  ...
  if (
    download.state == browser.downloads.State.COMPLETE &&
    !isAlreadyNotified
  ) {
    summary = 'Download Complete: "' + filename + '"';
    body = 'File: "' + filename + '"\n';
    body += 'Saved in ' + filepath.join(pathSep) + '\n';
    body += 'File Size: ' + download.fileSize;
  } else if (download.error != null && !isAlreadyNotified) {
    if (
      download.error == browser.downloads.InterruptReason.USER_SHUTDOWN ||
